...
NICHES = load_niches("/home/claude/promo-radar/config/niches.yaml")
def settings(**kw):
    return Settings(_env_file=None, amazon_credential_id="i", amazon_credential_secret="s", amazon_rps=1000, **kw)
...
rec("DISP-03","Timeout da API na coleta", "error" in r, f"capturado em {dt:.2f}s; timeout não tem retry (só 429/5xx têm)", "media")
...
t=time.perf_counter(); code = c.get(f"/posts/{pid}/send", auth=("admin","troque-esta-senha")).status_code; dt=time.perf_counter()-t
rec("DISP-05","Botão Enviar com a API fora do ar", code < 500, f"HTTP {code} após {dt:.1f}s: a tela de envio quebra em vez de avisar", "alta")
code = c.post(f"/posts/{pid}/refresh", auth=("admin","troque-esta-senha")).status_code
rec("DISP-06","Botão Revalidar com a API fora do ar", code < 500, f"HTTP {code}", "media")
# A7 monitor com API fora: exceção sobe para o agendador
try:
    good.mark_sent(pid); good.monitor_sent(); ok=True; det="tratado"
except Exception as e:
    ok=False; det=f"exceção não tratada ({type(e).__name__}); o APScheduler registra e tenta de novo na próxima hora"
...
db = DB("/home/claude/qa/run/race.db")
...
json.dump(R, open("/home/claude/qa/avail.json","w"), ensure_ascii=False, indent=1)
